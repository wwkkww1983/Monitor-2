...
long vartol(const _variant_t& var)
{
	...
	case VT_LPSTR://字符串
	case VT_LPWSTR://字符串
		value = _wtol((LPCTSTR)(_bstr_t)var);
		break;
	case VT_NULL:
	case VT_EMPTY:
		value = 0;
...
BOOL CAdoConnection::Open(LPCTSTR lpszConnect, long lOptions)
{
	...
	{
		// 连接数据库 ---------------------------------------------
		return (m_pConnection->Open(_bstr_t(LPCTSTR(m_strConnect)), _T(""), _T(""), lOptions) == S_OK);
	}
	catch (_com_error e)
	{
		TRACE(_T("Warning: 连接数据库发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
		TRACE(_T("%s\r\n"), GetLastErrorText());
//		::PostMessage(AfxGetMainWnd()->GetSafeHwnd(), MSG_HGLASER, MSG_TEXT_DEBUG,(LPARAM)(LPCTSTR)(e.Description()));
...
//		::PostMessage(AfxGetMainWnd()->GetSafeHwnd(), MSG_HGLASER, MSG_TEXT_DEBUG,(LPARAM)(LPCTSTR)(_T("Warning: 连接数据库时发生未知错误:")));
...
_RecordsetPtr CAdoConnection::Execute(LPCTSTR lpszSQL, long lOptions)
{
	...
	{
		USES_CONVERSION;
		return m_pConnection->Execute(_bstr_t(T2A(lpszSQL)), NULL, lOptions);
	}
	catch (_com_error e)
	{
